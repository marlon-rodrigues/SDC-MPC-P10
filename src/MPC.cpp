...
/* Solver takes 1 vector. This is to create an index to access variables in that vector. */
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Reduce gap between actuators (sequence)
    for(int t=0; t<(N-2); t++) {
    ...
    for(int i=0; i<(N-1); i++) {
      ...
      // The idea is to constraint this value to be 0. 
      // NOTE: use of 'AD<double>' and use of 'CppAD' is so CppAD can compute derivatives and pass
      // these to the solver.
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Setall non-actuators upper and lowerlimits
  ...
  // Acceleration/deccelaration upper and lower limits
  ...
  cout << "Steering angle " << solution.x[delta_start] << " Throttle " << solution.x[a_start] << endl;
